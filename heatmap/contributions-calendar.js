    ...
    class extends HTMLElement {
      ...
      count_contributions(date, contributions) {
        ...
        for (let i = 0; i < contributions.length; i++) {
          ...
          if (
            contribution_timestamp_in_millis >= start_date_in_millis &&
            contribution_timestamp_in_millis < end_date_in_millis
          ) {
            count += 1;
          }
        }
      ...
      build_day(date_contribution, week_number, day_number) {
        ...
        const day_template = parser
          .parseFromString(day_template_string, "text/html")
        ...
        if (date_contribution.count) {
          let percentage =
            date_contribution.count / this.MAX_CONTRIBUTIONS_NUMBER;
      ...
      build_week(week, week_number) {
        ...
        const parser = new DOMParser();
        const week_template = parser
          .parseFromString(week_template_string, "text/html")
          .querySelector("#week-template");
      ...
      build_container(days_array, start_date, number_of_weeks) {
        ...
        const container_template = parser.parseFromString(
          container_template_string,
          "text/html"
        );
      ...
      clickListener(event, detail) {
        ...
        if (rootElement.className === "day") {
          ...
          rootElement.dispatchEvent(
            new CustomEvent("contributions-calendar-day-selected", {
              detail: {
                date: date,
      ...
      draw() {
        ...
        let days_array = this.build_days_array(
          this.CONTRIBUTIONS,
          this.START_DATE,
          this.NUMBER_OF_WEEKS
        );
        let container_clone = this.build_container(
          days_array,
          this.START_DATE,
          this.NUMBER_OF_WEEKS
        );
